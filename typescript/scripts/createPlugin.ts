...
function getWalletClientImport(type: PluginOptions["type"]): string {
    switch (type) {
        case "evm":
            return 'import type { EVMWalletClient } from "@frog-sdk/wallet-evm";';
        case "solana":
            return 'import type { SolanaWalletClient } from "@frog-sdk/wallet-solana";';
        case "any":
            return 'import { WalletClientBase } from "@frog-sdk/core";';
    }
}
...
function getDependencies(type: PluginOptions["type"]): Record<string, string> {
    const deps: Record<string, string> = {
        "@frog-sdk/core": "workspace:*",
        zod: "catalog:",
    };

    switch (type) {
        case "evm":
            deps["@frog-sdk/wallet-evm"] = "workspace:*";
            break;
        case "solana":
            deps["@frog-sdk/wallet-solana"] = "workspace:*";
            break;
    }
...
function createPackageJson(options: PluginOptions): string {
    ...
    const packageJson = {
        ...
        repository: {
            type: "git",
            url: "git+https://github.com/frog-sdk/frog.git",
        },
        license: "MIT",
        bugs: {
            url: "https://github.com/frog-sdk/frog/issues",
        },
...
function createParametersContent(): string {
    return `import { createToolParameters } from "@frog-sdk/core";
import { z } from "zod";
...
function createServiceContent(options: PluginOptions): string {
    ...
    return `import { Tool } from "@frog-sdk/core";
${walletClientImport}
...
function createPluginContent(options: PluginOptions): string {
    ...
    return `import { PluginBase } from "@frog-sdk/core";
...
program
    .name("createPlugin")
    .description("Create a new frog SDK plugin")
