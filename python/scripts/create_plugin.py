...
frog-sdk = { path = "../../frog-sdk", develop = true }'''
    ...
    if is_evm:
        ...
        dev_dependencies += '\nfrog-sdk-wallet-solana = { path = "../../wallets/solana", develop = true }'
    
    toml_content = f'''[tool.poetry]
name = "frog-sdk-plugin-{plugin_name}"
version = "0.1.0"
description = "frog plugin for {plugin_name}"
authors = ["Your Name <your_email@example.com>"]
readme = "README.md"
keywords = ["frog", "sdk", "agents", "ai", "{plugin_name}"]
homepage = "https://ohmyfrog.dev/"
repository = "https://github.com/frog-sdk/frog"
packages = [
    {{ include = "frog_plugins/{plugin_name}" }},
]
...
def create_parameters_file(frog_plugins_dir: Path, plugin_name: str) -> None:
    """Create the parameters.py file with example parameters in the frog_plugins directory."""
...
'''
    ...
    with open(frog_plugins_dir / "parameters.py", "w") as f:
...
def create_service_file(frog_plugins_dir: Path, plugin_name: str, is_evm: bool, is_solana: bool = False) -> None:
    ...
    service_content = '''from frog.decorators.tool import Tool
from .parameters import ExampleQueryParameters, ExampleActionParameters
'''

    # Add chain-specific imports if needed
    if is_evm:
        service_content += '''from frog_wallets.evm import EVMWalletClient

'''
    elif is_solana:
        service_content += '''from frog_wallets.solana import SolanaWalletClient
...
'''
    ...
    with open(frog_plugins_dir / "service.py", "w") as f:
        f.write(service_content)

def create_init_file(frog_plugins_dir: Path, plugin_name: str, is_evm: bool, is_solana: bool = False) -> None:
...
from frog.classes.plugin_base import PluginBase
...
'''.format(
    ...
    with open(frog_plugins_dir / "__init__.py", "w") as f:
        f.write(init_content)

def main():
    """Main function to create a new frog plugin.
    
    This script generates a new plugin package for the frog SDK with a standardized structure.
    ...
    """
    parser = argparse.ArgumentParser(description="Create a new frog plugin.")
    ...
    # Create frog_plugins directory and its contents
    frog_plugins_dir = plugin_dir / "frog_plugins" / plugin_name
    frog_plugins_dir.mkdir(parents=True, exist_ok=True)
    
    # Create all plugin files
    create_parameters_file(frog_plugins_dir, plugin_name)
    create_service_file(frog_plugins_dir, plugin_name, is_evm, is_solana)
    create_init_file(frog_plugins_dir, plugin_name, is_evm, is_solana)
    ...
    additional_deps = ''
    if is_evm:
        additional_deps = '# Install required wallet dependency\npoetry add frog-sdk-wallet-evm'
    elif is_solana:
        additional_deps = '# Install required wallet dependency\npoetry add frog-sdk-wallet-solana'
    
    readme_content = f"""# {plugin_name} Plugin for frog SDK

A plugin for the frog SDK that provides {plugin_name} functionality.

## Installation

```bash
# Install the plugin
poetry add frog-sdk-plugin-{plugin_name}
...
```python
from frog_plugins.{plugin_name} import {python_name}, {class_base}PluginOptions
