...
from frog import ToolBase, WalletClientBase, get_tools


def get_on_chain_tools(wallet: WalletClientBase, plugins: List[Any]) -> List[BaseTool]:
    """Create LangChain tools from frog tools.
    ...
    Returns:
        List of LangChain Tool instances configured with the frog tools
    """
    ...
    langchain_tools = []
    for t in tools:
        # Create a LangChain Tool for each frog tool
        tool = StructuredTool(
            name=t.name,
